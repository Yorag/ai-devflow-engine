...
Main-branch PR review found two Important issues: stage_prompt_fragment rendered into the system message but was absent from ContextEnvelope/manifest records, and size guard token accounting did not include the new fragment. Both were fixed by mapping stage_prompt_fragment into the stage_contract prompt-backed block group.
PR review fix RED: backend/tests/context/test_context_envelope_builder.py::test_stage_execution_builds_envelope_manifest_messages_and_persists_context_manifest failed with IndexError because result.envelope.stage_contract[1] did not exist.
PR review fix GREEN: C:\Users\lkw\Desktop\github\agent-project\ai-devflow-engine\.venv\Scripts\python.exe -m pytest backend/tests/context/test_context_envelope_builder.py::test_stage_execution_builds_envelope_manifest_messages_and_persists_context_manifest -q -> 1 passed in 0.14s.
Pinned accounting coverage: C:\Users\lkw\Desktop\github\agent-project\ai-devflow-engine\.venv\Scripts\python.exe -m pytest backend/tests/context/test_context_envelope_builder.py::test_stage_prompt_fragment_is_counted_as_pinned_context -q -> 1 passed in 0.20s.
Impacted context verification: C:\Users\lkw\Desktop\github\agent-project\ai-devflow-engine\.venv\Scripts\python.exe -m pytest backend/tests/context/test_context_envelope_builder.py backend/tests/context/test_context_compression.py -q -> 22 passed in 0.23s.
```
