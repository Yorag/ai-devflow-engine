...
| AL02 | `feat/al-projections-streams` | Q3.1, Q3.2, Q3.2a, Q3.3, Q3.4, Q3.4a, E3.2, L3.1, L4.2, Q3.4b | claimed | Projection services、SSE、run/stage log query、audit query route | 查询投影、实时流和轻查询 API |
...
| AL06 | `feat/al-frontend-runtime-ui` | F3.1, F3.2, F3.3, F3.4, F3.5, F3.6, F3.7, H4.2, F4.1, F4.2, F4.3, F4.3a, F4.4, F4.4a, F4.4b, F5.1, F5.2a, F5.2b | claimed | frontend workspace store、SSE reducer、Feed、StageNode、Inspector、Composer、Approval、Tool Confirmation、Delivery UI | 前端运行工作台和交付展示 |
...
| AL06-F4.4b-RERUN-FOCUS | F4.4b | AL06 | `feat/al-frontend-runtime-ui` | done | 534c492 | fa64197 | `docs/plans/acceleration/reports/AL06-F4.4b-RERUN-FOCUS.md` | RerunAction now consumes backend-shaped `{ session, run }` rerun responses and focuses `response.run.run_id`; focused frontend tests, full frontend suite, frontend build, and route-fixture Playwright control-flow verification passed on integration branch. |
...
| AL06-F5.2b | F5.2b | AL06 | `feat/al-frontend-runtime-ui` | done | b509007 | b6b535d | `docs/plans/acceleration/reports/AL06-F5.2b.md` | Backend-shaped `git_auto_delivery` `delivery_result` and `DeliveryResultDetailProjection` payload proof integrated; Feed, shared delivery block, Inspector mock route, and build verification passed on integration. |
...
| AL04-QA-E2E-V6.3-HARNESS | A4.3a | AL04 | `feat/al-tools-deterministic-delivery` | done | c7b5877 | ca2dbaf | `docs/plans/acceleration/reports/AL04-QA-E2E-V6.3-HARNESS.md` | Deterministic runtime E2E advancement harness integrated for V6.3 live unblock; focused harness, full API flow, OpenAPI, and human-loop API regression passed on integration branch. Worker full backend suite evidence records 1329 passed, 3 warnings. |
...
| AL06-QA-E2E-V6.2 | V6.2 | AL06 | `feat/al-frontend-runtime-ui` | integrated | b94e8a5 | e15c977 | `docs/plans/acceleration/reports/AL06-QA-E2E-V6.2.md` | Frontend `New session` action now calls the real create-session API path and updates workspace/session query state; `npm --prefix frontend run test -- --run WorkspaceShell`, full frontend suite, and frontend build passed on integration branch. Final V6.2 completion is covered by `QA-E2E-V6.2-FINAL`. |
| QA-E2E-V6.2 | V6.2 | QA-E2E | `test/qa-e2e-regression` | integrated | 2e682ec | ed7c8d9 | `docs/plans/acceleration/reports/QA-E2E-V6.2.md` | Mock-ready Playwright success-path checkpoint integrated on `integration/function-one-acceleration`; `npm --prefix e2e run test -- function-one-full-flow.spec.ts` passed. Superseded by final UI-created-session coverage in `QA-E2E-V6.2-FINAL`. |
| QA-E2E-V6.2-FINAL | V6.2 | QA-E2E | `test/qa-e2e-regression` | done | e668943 | 98e1ef9 | `docs/plans/acceleration/reports/QA-E2E-V6.2-FINAL.md` | Final Playwright success-path checkpoint integrated on `integration/function-one-acceleration`; `npm --prefix e2e run test -- function-one-full-flow.spec.ts`, `npm --prefix frontend run test -- --run WorkspaceShell`, full frontend suite, and frontend build passed after merge. Platform and split V6.2 status are `[x]`. |
| QA-E2E-V6.3 | V6.3 | QA-E2E | `test/qa-e2e-regression` | integrated | a48ea61 | 575088d | `docs/plans/acceleration/reports/QA-E2E-V6.3.md` | Mock-ready Playwright manual-intervention checkpoint integrated on `integration/function-one-acceleration`; `npm --prefix e2e run test -- function-one-full-flow.spec.ts function-one-control-flow.spec.ts` passed. Platform and split status remain `[/]` because this checkpoint uses route-level projection fixtures rather than live backend orchestration, persistence, and SSE delivery. |
| QA-E2E-V6.3-LIVE | V6.3 | QA-E2E | `test/qa-e2e-regression` | done | f8ae3e7 | 583e6f5 | `docs/plans/acceleration/reports/QA-E2E-V6.3-LIVE.md` | Live backend-backed Playwright manual-intervention checkpoint integrated after AL06 F4.4b; route-fixture V6.3, live backend V6.3, and V6.2 success-path regressions passed on integration branch. Platform and split V6.3 status are `[x]`. |
| QA-ERROR-V6.6 | V6.6 | QA-ERROR | `test/qa-error-regression` | done | 2e682ec | 3ff7cc3 | `docs/plans/acceleration/reports/QA-ERROR-V6.6.md` | Error-code catalog, API conflict mapping, frontend ErrorState presentation, focused E2E-adjacent UI regressions, impacted backend / frontend verification, full backend suite, full frontend suite, and frontend build passed on integration branch. |
...
| AL02 | Q3.1 -> Q3.2 -> Q3.2a -> Q3.3 -> Q3.4 -> Q3.4a -> E3.2 -> L3.1 -> L4.2 -> Q3.4b |
...
| AL06 | F3.1 -> F3.2 -> F3.3 -> F3.4 -> F3.5 -> F3.6 -> F3.7 -> H4.2 -> F4.1 -> F4.2 -> F4.3 -> F4.3a -> F4.4 -> F5.1 -> F5.2a -> F5.2b -> F4.4a -> F4.4b |
| QA | V6.1 -> V6.4 -> V6.5 |
| QA-E2E | V6.2 -> V6.3 |
| QA-ERROR | V6.6 |
| QA-CONFIG | V6.8 |
