...
**验证摘要**：实施计划 `docs/plans/implementation/r3.2-start-first-run.md` 与 R3.2a publication boundary 已在 integration 分支完成收敛。`uv run python -m pytest backend/tests/services/test_start_first_run.py backend/tests/api/test_session_message_api.py backend/tests/api/test_startup_publication_visibility.py backend/tests/services/test_publication_boundary.py -q` 通过 24 个 focused tests，覆盖首条 `new_requirement` 启动首个 run、session message API、startup publication visibility 和 publication boundary。
...
**计划周期**：Week 4
**状态**：`[x]`
**目标**：以已集成的 R3.1 状态机规则和 H4.7 外部 rerun 命令实现覆盖重新尝试领域规则与内部创建基础，使 failed / terminated run 后的新 PipelineRun 语义受状态机约束，并通过完整命令、事件、投影和 API 路由暴露。
**实施追踪**：本任务未形成独立 `docs/plans/implementation/r3.3-rerun-creation.md` worker checkpoint；原 R3.3 内部 rerun 基础已由 `docs/plans/implementation/r3.1-run-state-machine.md` 的状态机规则和 `docs/plans/implementation/h4.7-rerun-command-run-boundary.md` 的 rerun 命令边界吸收完成。
**验证摘要**：H4.7 integration checkpoint `58a020b` 已覆盖 rerun creation 语义；`uv run python -m pytest backend/tests/services/test_rerun_command_projection.py backend/tests/api/test_rerun_command_api.py -q` 通过 18 个 focused rerun tests。R3.1 状态机验证由 `backend/tests/domain/test_run_state_machine.py` 覆盖。

**修改文件列表**：
- Modify: `backend/app/services/runs.py`
- Modify: `backend/app/domain/state_machine.py`
...
## Q3.2a Run 状态摘要查询 API

**计划周期**：Week 5
**状态**：`[x]`
**目标**：补齐 `GET /api/runs/{runId}` 查询接口，使前端、QA OpenAPI 回归和后端只读查询契约可读取单个 run 的领域状态摘要，而不暴露 raw graph thread 详情。
**实施计划**：`docs/plans/implementation/q3.2a-run-summary-query-api.md`

**修改文件列表**：
- Modify: `backend/app/schemas/run.py`
- Modify: `backend/app/services/projections/timeline.py` 或 Create: `backend/app/services/projections/run_summary.py`
- Modify: `backend/app/api/routes/query.py`
- Modify: `backend/tests/projections/test_timeline_projection.py` 或 Create: `backend/tests/projections/test_run_summary_projection.py`
- Modify: `backend/tests/api/test_query_api.py`

**实现类/函数**：
- `TimelineProjectionService.get_run_summary()` 或 `RunSummaryProjectionService.get_run_summary()`
- `get_run_summary()`

**验收标准**：
- `GET /api/runs/{runId}` 返回领域层 run 状态摘要，不返回 raw graph thread、checkpoint 或 LangGraph 内部详情。
- 响应至少包含 `RunSummaryProjection` 的全部字段，以及该 run 的 `current_stage_run_id`。
- 目标 run 必须通过现有 publication boundary 和历史可见性规则校验；不可见、缺失或跨对象不一致时返回统一 404 错误。
- `current_stage_type` 与现有 workspace/timeline 投影口径一致；`current_stage_run_id` 只来自目标 run 本身，不从其它 run 泄漏。
- API 测试必须断言 `GET /api/runs/{runId}` 的响应 Schema、主要错误响应和 OpenAPI path/method 已进入 `/api/openapi.json`。
- 修复后必须能解除 `QA-V6.4` 对 `/api/runs/{runId}` 的 OpenAPI 缺失阻塞。

**测试方法**：
- `pytest backend/tests/projections/test_timeline_projection.py -v` 或 `pytest backend/tests/projections/test_run_summary_projection.py -v`
- `pytest backend/tests/api/test_query_api.py -v`
