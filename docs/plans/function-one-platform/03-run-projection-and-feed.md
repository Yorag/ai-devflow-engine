...
**计划周期**：Week 4
**状态**：`[ ]`
**目标**：实现 `POST /api/sessions/{sessionId}/messages` 中 `new_requirement` 语义，在同一启动事务中创建首个 PipelineRun、模板快照、Provider 与模型绑定快照、RuntimeLimitSnapshot、ProviderCallPolicySnapshot、GraphDefinition、首个 GraphThread、首个 workspace_ref、首条消息事件和初始 Requirement Analysis StageRun。
...
**计划周期**：Week 5-6
**状态**：`[ ]`
**目标**：修复顶层 `tool_confirmation` 在 query / workspace / timeline / SSE 中的拒绝后续语义契约，使前端无需读取 Inspector 或推断 run 终态即可稳定展示 deny 后运行结果。
...
- 若 H4.4a 当前实现只能提供 `alternative_path_summary`，仍不能稳定区分 `run_failed` 与 `awaiting_run_control`，则本切片必须停止并回报需要先补 runtime source，不得以猜测映射完成该契约。
