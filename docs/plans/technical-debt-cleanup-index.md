...
| TD-004 | P1 | accepted-boundary | `docs/plans/acceleration/reports/AL01-E3.1.md` | EventStore sequence uniqueness is process-local. Current V1 production/display deployment is constrained to a single backend writer, so multi-worker durable sequence allocation is outside the current production gate. | Keep the single-writer deployment boundary explicit and retain current EventStore regression coverage. Reopen before multi-worker API/runtime deployment, parallel command workers, or any production claim that multiple processes can write events concurrently. | `uv run python -m pytest backend/tests/events/test_event_store.py -q` | Batch 3 |
...
| TD-006 | P2 | resolved-by-verification | `docs/plans/acceleration/reports/AL01-C2.9b.md`; `docs/plans/implementation/td-006-project-removal-coverage.md` | Project removal now has service and API regression coverage for a real pending-startup publication blocker and forced runtime-barrier acquisition failure. Current code did not need production changes for this debt item. | No further cleanup in Batch 2. Reopen only if Project removal changes bypass startup-publication blocking, runtime barrier acquisition, or no-mutation failure behavior. | `uv run python -m pytest backend/tests/services/test_project_service.py backend/tests/api/test_project_remove_api.py backend/tests/api/test_project_api.py -q` | Batch 2 |
...
| TD-008 | P2 | resolved-by-verification | `docs/plans/acceleration/reports/AL02-Q3.4.md`; `docs/plans/implementation/td-008-control-item-malformed-payload.md` | Malformed and contract-invalid `control_item` event payload handling now has focused control item detail projection regression coverage. The regression exposed that schema-valid wrong-run payloads could replace a valid control event, so `_control_event()` now also requires parsed `control_item.run_id` to match the visible run. | No further cleanup in Batch 2. Reopen if control item projection begins accepting broader event payload sources or stops validating parsed payload identity against the current run. | `uv run python -m pytest backend/tests/projections/test_control_item_detail_projection.py -q` | Batch 2 |
| TD-009 | P2 | resolved-by-verification | `docs/plans/acceleration/reports/AL03-H4.5.md`; `docs/plans/implementation/td-009-timeline-pause-resume-replay.md` | Timeline API now explicitly replays paused and resumed tool-confirmation state through `GET /api/runs/{runId}/timeline`, including disabled actionability while paused and restored actionability after resume. | No further cleanup in Batch 2. Reopen if pause/resume starts producing top-level control/timeline entries or if timeline no longer reflects current waiting checkpoint actionability. | `uv run python -m pytest backend/tests/services/test_pause_resume.py backend/tests/api/test_run_timeline_api.py -q` | Batch 2 |
...
| TD-011 | P2 | resolved-by-verification | `docs/plans/acceleration/reports/QA-E2E-V6.3-LIVE.md`; `docs/plans/acceleration/reports/QA-RELEASE-V6.7.md`; `docs/plans/implementation/td-011-live-backend-e2e-release-entry.md` | Live backend Playwright coverage now has a stable release/display verification entry: `npm --prefix e2e run test:live`. The script enables the live backend harness and runs the live control-flow spec through the project default E2E ports. | No further TD-011 cleanup. Reopen if release/display verification stops running `test:live`, the script stops enabling the live backend harness, or the live control-flow spec is removed from the stable command. | `npm --prefix e2e run test:live` | Batch 2 |
...
| TD-016 | P0 | open | `docs/specs/function-one-product-overview-v1.md`; `docs/plans/function-one-platform/09-regression-hardening-and-logs.md`; `docs/plans/acceleration/reports/AL05-A4.9d.md`; `docs/plans/acceleration/reports/AL04-W5.0b.md`; `docs/plans/acceleration/reports/AL04-W5.4.md`; `docs/plans/implementation/td-016-real-model-stage-handoff.md`; 2026-05-07 live-model smoke runs | Real-model website text-change smoke runs show the formal stage chain is not yet coherently bound end to end. `Solution Design` is specified to produce an executable task plan, but `Code Generation` and `Test Generation & Execution` still behave like independent exploratory ReAct loops. Tool prompt guidance, stage allowed tools, risk classification, bash allowlist, OS command availability, artifact handoff, and test evidence selection are not governed by one executable contract. This produced repeated drift: code generation clarification/failures after upstream analysis, test-stage exploration of package scripts for a one-line text change, and inconsistent treatment of grep/bash verification commands. | Execute the focused cleanup plan in `docs/plans/implementation/td-016-real-model-stage-handoff.md`: make `SolutionDesignArtifact` carry a machine-readable execution plan, require downstream stages to consume that plan before exploratory tool use, define one shared verification-command policy used by prompt guidance/risk/bash execution, and add a real-model API smoke for the platform website text-change scenario. Do not continue demo-specific hardcoded command rewrites. | New focused real-model smoke, for example: start local API, submit `请只修改 frontend/src/pages/HomePage.tsx...`, assert stages reach delivery or an intentional approval gate, assert only the target file changes in the run workspace, and assert test evidence comes from the plan-defined verification. Keep existing deterministic E2E as separate regression coverage. | Batch 1 |
...
### 2026-05-06 Batch 2 Parallel Cleanup

- TD-006 verification passed: `uv run python -m pytest backend/tests/services/test_project_service.py backend/tests/api/test_project_remove_api.py backend/tests/api/test_project_api.py -q` returned `21 passed`.
- TD-008 verification passed: `uv run python -m pytest backend/tests/projections/test_control_item_detail_projection.py -q` returned `7 passed`.
- TD-009 verification passed: `uv run python -m pytest backend/tests/api/test_run_timeline_api.py backend/tests/services/test_pause_resume.py -q` returned `14 passed`.

### 2026-05-06 Production-Path Verification Cleanup

- TD-004 is accepted as a single-writer V1 deployment boundary. It is not a current production blocker while the system is not deployed with multiple backend writer processes.
- TD-011 verification passed: `npm --prefix e2e run test:live` returned `2 passed`, covering the live backend control-flow spec through real REST projections and SSE.

## Next Recommended Cleanup Slice

Continue with production-path validation debt before lower-risk backend infrastructure debt:

1. Close TD-016 first. It is the current blocker for a trustworthy real-model demo and should be fixed at the stage handoff / executable-plan boundary, not by adding demo-specific command rewrites.
2. Decide whether TD-012 should be closed next by adding unique runtime-root support or cleanup behavior for live E2E runs.
3. Close the remaining TD-003 rich metadata decision by choosing whether `ClarificationRecord` gets schema-owned metadata fields, or whether rich clarification metadata is owned by `StageArtifact.process` and referenced through `payload_ref`.
4. Defer runtime schema-column work until TD-013 has a migration baseline, unless the cleanup slice deliberately includes both the model change and the baseline migration.
5. Treat TD-002 and TD-004 as accepted boundaries for the current display-solidification phase, but reopen them before multi-worker, external-runtime, crash-recovery, or multi-writer production claims.

Reason: the current production/display risk is proving the real frontend-backend path and keeping its local verification reliable. Multi-worker sequencing is documented as a boundary, not a near-term blocker.
