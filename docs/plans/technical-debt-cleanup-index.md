...
| TD-002 | P0 | open | `docs/plans/acceleration/reports/AL03-H4.1.md` | Clarification command evidence records residual architecture risk. Current tests verify required audit failures block runtime create/resume side effects and runtime resume failure rolls back uncommitted domain/event changes, but runtime side effects and multi-SQLite commits are still not one durable atomic unit. | Decide the production boundary: either document the accepted single-process/runtime-port recovery limit for display stabilization, or design an outbox/idempotent runtime command before multi-worker production. | `uv run python -m pytest backend/tests/services/test_clarification_flow.py backend/tests/services/test_runtime_orchestration_boundary.py backend/tests/schemas/test_run_feed_event_schemas.py -q` | Batch 1 |
| TD-003 | P1 | open | `docs/plans/acceleration/reports/AL03-H4.1.md`; `docs/specs/function-one-backend-engine-design-v1.md` | Clarification backend still has model/spec drift: the spec lists `ClarificationRecord.status`, while the runtime model infers pending/answered from `answer` and `answered_at`. Rich Requirement Analysis metadata is not schema-owned on `ClarificationRecord`; current implementation only has `payload_ref` and later `StageArtifact` process capacity. | Do not add runtime columns until the migration baseline and source-of-truth decision are settled. Next action is a focused design patch: either approve `StageArtifact.process` / `payload_ref` as the owner for rich metadata, or add model fields with migration and regression coverage. | `uv run python -m pytest backend/tests/services/test_clarification_flow.py backend/tests/schemas/test_run_feed_event_schemas.py -q` | Batch 1 |
...
1. Close TD-002 by explicitly choosing the accepted production boundary for clarification command recovery.
2. Close TD-003 by deciding whether `ClarificationRecord` gets schema-owned status and metadata fields, or whether rich clarification metadata is owned by `StageArtifact.process` and referenced through `payload_ref`.
