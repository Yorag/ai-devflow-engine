...
| DB06 | `feat/persistence-boundaries` | C1.5-C1.9, L1.2 | DB04, DB05 | S | ready_for_review | 数据库 session、control/runtime/graph/event/log models、Alembic migration 链 | 多 SQLite 职责库、模型和迁移边界 |
...
| DB10 | `feat/frontend-control-plane-client` | F2.1, F2.2 | DB03, DB04, DB05 | Y | ready_for_review | 前端 API client 类型入口、mock fixtures | 前端 API client、mock fixtures 和 query hooks |
