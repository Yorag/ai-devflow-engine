...
| DB02 | `feat/backend-runtime-bootstrap` | B0.2, B0.3, L0.1 | DB01 | S | ready_for_review | `backend/app/api/*`, `error_codes.py`, settings、日志目录预检 | FastAPI、统一错误响应、启动配置和运行数据目录 |
| DB03 | `feat/frontend-spa-baseline` | F0.1 | DB01 | G | ready_for_review | `frontend/`, 前端测试入口 | 前端 SPA、测试基线和首个设计质量门 |
| DB04 | `feat/core-schema-contracts` | C1.1-C1.4 | DB02 | S | planned | 全局枚举、核心 Schema、事件 payload、Projection Schema | 全局枚举、控制面、Run/Feed/Event 和 Inspector 契约 |
| DB05 | `feat/runtime-prompt-log-contracts` | C1.10, C1.10a, L1.1 | DB04 | Y | planned | 运行设置 Schema、PromptAsset Schema、TraceContext | 运行设置、提示词资产和日志审计契约 |
...
| DB10 | `feat/frontend-control-plane-client` | F2.1, F2.2 | DB03, DB04, DB05 | Y | planned | 前端 API client 类型入口、mock fixtures | 前端 API client、mock fixtures 和 query hooks |
| DB11 | `feat/frontend-workspace-template-ui` | F2.3-F2.6 | DB10 | Y | planned | App shell、前端全局 store、设置弹窗、模板 UI | Workspace shell、设置弹窗和模板交互 |
