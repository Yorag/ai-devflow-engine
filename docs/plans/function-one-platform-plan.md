...
| R3.2 | 首条需求启动首个 run | Week 4 | [x] | 后端核心 | [03](function-one-platform/03-run-projection-and-feed.md#r32) |
| R3.3 | 重新尝试领域规则与内部创建基础 | Week 4 | [x] | 后端核心 | [03](function-one-platform/03-run-projection-and-feed.md#r33) |
...
| Q3.2a | Run 状态摘要查询 API | Week 5 | [x] | 后端 | [03](function-one-platform/03-run-projection-and-feed.md#q32a) |
...
| L4.2 | 审计日志查询 API | Week 6 | [x] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#l42) |
...
| F4.4a | RerunAction 真实 retry_action 契约重连 | Week 11 | [ ] | 前端 | [04](function-one-platform/04-human-loop-and-runtime.md#f44a) |
...
| A4.3a | deterministic runtime E2E 推进 harness | Week 11 | [x] | 后端测试 | [05](function-one-platform/05-deterministic-runtime-and-demo-delivery.md#a43a) |
...
| A4.11 | 自动回归控制条目与超限失败 | Week 8-9 | [x] | 后端 | [06](function-one-platform/06-langgraph-provider-context-stage-agent.md#a411) |
...
| D5.1 | read_delivery_snapshot 与交付快照读取 | Week 10 | [x] | 后端 | [08](function-one-platform/08-git-delivery-and-result-display.md#d51) |
| D5.2 | prepare_branch 与 create_commit | Week 10 | [x] | 后端 | [08](function-one-platform/08-git-delivery-and-result-display.md#d52) |
| D5.3 | push_branch 与 create_code_review_request | Week 10 | [x] | 后端 | [08](function-one-platform/08-git-delivery-and-result-display.md#d53) |
| D5.4 | git_auto_delivery 编排与 snapshot readiness 测试 | Week 10 | [x] | 后端 | [08](function-one-platform/08-git-delivery-and-result-display.md#d54) |
| F5.1 | 工具调用、Diff 与测试结果展示 | Week 9-10 | [x] | 前端 | [08](function-one-platform/08-git-delivery-and-result-display.md#f51) |
| F5.2a | demo_delivery 结果展示 | Week 9 | [x] | 前端 | [08](function-one-platform/08-git-delivery-and-result-display.md#f52a) |
...
| V6.8 | 配置边界与运行快照回归 | Week 12 | [x] | 跨端 | [09](function-one-platform/09-regression-hardening-and-logs.md#v68) |
...
本节索引功能一剩余实现的加速执行控制表。完整调度规则位于 [function-one-acceleration-execution-plan.md](function-one-acceleration-execution-plan.md)，用于子任务依赖队列、lane 冲突域分支、共享写集 owner、checkpoint snapshot 和 integration checkpoint 控制。实时 claim 状态由 git common dir 下的 shared coordination store 承载。
...
| 文档 | 内容 | 读取时机 |
| --- | --- | --- |
| [function-one-acceleration-execution-plan.md](function-one-acceleration-execution-plan.md) | Mode、Lane Registry、checkpoint snapshot、Lane Queue、Shared Ownership、Start Gate、Merge Gate、Integration Checkpoints | 主协调会话读取静态 lane / queue / owner 规则、生成 worker prompt、检查 owner 冲突、执行 integration checkpoint |
...
主线进度事实仍由本文件第 9 节和对应 split-plan 任务状态共同构成；加速执行控制表维护 lane、共享写集、checkpoint snapshot 和 integration checkpoint 规则。实时 claim、reported、implemented、mock_ready、blocked 状态由 shared coordination store 维护。Worker 分支不得直接更新全局最终完成状态，最终状态由主协调会话在 integration checkpoint 通过后统一收敛。
