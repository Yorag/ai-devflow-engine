...
> 执行模型：前后端并行，后端契约、runtime boundary 与 ToolProtocol 先行，Workspace tools 基座先于正式 runtime 消费，deterministic runtime 先打通稳定链路，LangGraph runtime 作为正式编排路径
...
- ToolProtocol 先行：正式 runtime、Provider adapter、Workspace tools 与 Delivery tools 只能消费抽象 `ToolProtocol` 和 `ToolRegistry`，不允许先接入临时工具函数或尚未实现的具体 delivery tool 实例。
...
| Week 7 | deterministic 与工具基座 | RuntimeEngine、deterministic runtime、ToolProtocol、Workspace tools 基座、工具审计、demo_delivery | 前端完整流程联调 | 不依赖真实模型跑通 demo_delivery，终态回放可用，工具协议和审计不临时分叉 |
| Week 8 | LangGraph 正式路径 | Graph compiler、interrupt、checkpoint、Provider adapter、自动回归、模型/runtime 日志 | Runtime 错误态与状态呈现 | deterministic/runtime 双路径可切换，LangGraph 消费抽象 ToolProtocol 与已注册 workspace/fake 工具 |
...
| W5.2 | 文件工具 read/write/edit/list | Week 7 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w52) |
| W5.3 | search 工具 | Week 7-8 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w53) |
| W5.4 | shell 工具与日志审计记录 | Week 7-8 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w54) |
...
- ToolProtocol、ToolRegistry 与 Workspace tools 基座完成，deterministic runtime 不使用临时工具接口。
- Workspace tools 默认排除 `.runtime/logs` 与平台运行数据目录；文件写入、编辑、shell 和测试执行具备运行日志与审计引用。
...
- Provider adapter 和 LangGraph runtime 消费抽象 `ToolProtocol` 与已注册 workspace/fake 工具，不绑定 D5.1-D5.4 之前尚未实现的具体 delivery tool 实例。
- deterministic/runtime 双路径可切换，且 raw graph state 不进入产品 API。

### Week 9

- ChangeSet、ContextReference、PreviewTarget 边界落地。
- ChangeSet、diff、search 和工具展示均不把 `.runtime/logs` 或平台运行数据目录当作项目内容。
- 前端可展示工具调用、diff、测试结果。
...
- `git_auto_delivery` 通过 `read_delivery_channel` 读取已固化且字段完整的交付快照，不重新读取项目级最新 DeliveryChannel。
...
| ToolProtocol 漂移导致 runtime、Provider adapter 与 delivery tool 绑定口径不一 | W5.0 前置固定 `ToolProtocol` 与 `ToolRegistry`，W5.2-W5.4 和 D5.1-D5.4 只实现具体工具，LangGraph 和 Provider adapter 只能消费抽象协议 |
