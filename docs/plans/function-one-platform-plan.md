...
| Week 2 | 契约与持久化 | 枚举、Schema、多 SQLite、Alembic | API client、mock fixtures | Pydantic Schema 与投影契约可作为前端 mock 输入 |
| Week 3 | 控制面可用 | Project、Session、Template、Provider、DeliveryChannel API | Shell、设置弹窗、模板空态 | 控制面 OpenAPI 与已落地路由字段对齐 |
| Week 4 | Run 主链骨架 | Run 状态机、模板快照、GraphDefinition、StageArtifact | Workspace 页面、Run 分段基础展示 | draft、running 与基础阶段回放可用 |
| Week 5 | 投影与实时更新 | Workspace Projection、Timeline、Inspector、SSE | Narrative Feed、Inspector、SSE merge | 快照 + 增量一致 |
| Week 6 | 人工介入与 runtime 边界 | Runtime orchestration boundary、澄清、审批、暂停、恢复、终止、重新尝试、交付快照 gate | Approval Block、Composer lifecycle、重新尝试 UI | 人工介入命令只通过统一 runtime 边界推进 |
| Week 7 | deterministic 与工具基座 | RuntimeEngine、deterministic runtime、ToolProtocol、Workspace tools 基座、demo_delivery | 前端完整流程联调 | 不依赖真实模型跑通 demo_delivery，终态回放可用，且工具协议不临时分叉 |
| Week 8 | LangGraph 正式路径 | Graph compiler、interrupt、checkpoint、Provider adapter、自动回归 | Runtime 错误态与状态呈现 | deterministic/runtime 双路径可切换，LangGraph 消费抽象 ToolProtocol 与已注册 workspace/fake 工具 |
| Week 9 | 扩展边界与执行展示 | ChangeSet、ContextReference、PreviewTarget | diff、工具调用、测试结果展示 | 功能二预留边界与前端执行展示可验收 |
| Week 10 | 真实 Git 交付适配 | git_auto_delivery | 真实交付结果、交付失败态、历史详情 | git_auto_delivery 可测 |
                    ...
                    -> SSE / 前端状态合并
                      -> Narrative Feed / Inspector UI
                        -> Runtime orchestration boundary
                          -> Delivery snapshot gate
                            -> Human-in-the-loop commands
                              -> ToolProtocol / ToolRegistry
                                -> Workspace tools foundation
                                  -> deterministic runtime / demo_delivery
                                    -> LangGraph runtime / Provider adapter
                                      -> ChangeSet / ContextReference / PreviewTarget
                                        -> git_auto_delivery
...
| [01 工程基线与契约层](function-one-platform/01-foundation-and-contracts.md) | 工程基线、Schema 契约、数据库职责拆分 | B0.1, B0.2, F0.1, C1.1-C1.9 |
...
| [04 人工介入与执行内核](function-one-platform/04-human-loop-and-runtime.md) | Runtime orchestration boundary、澄清、审批、暂停恢复终止、交付快照、deterministic runtime、demo_delivery、LangGraph、Provider、自动回归 | A4.0-A4.11, H4.1-H4.7, F4.1-F4.4, D4.0-D4.3 |
| [05 工作区、真实 Git 交付与硬化](function-one-platform/05-workspace-delivery-and-hardening.md) | ToolProtocol、Workspace tools、ChangeSet、git_auto_delivery、E2E、OpenAPI、回归硬化 | W5.0-W5.6, D5.1-D5.4, F5.1-F5.2b, V6.1-V6.7 |
...
| C1.9 | event 与 audit 模型边界 | Week 2 | [ ] | 后端 | [01](function-one-platform/01-foundation-and-contracts.md#c19) |
...
| W5.4 | shell 工具与审计记录 | Week 7-8 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w54) |
...
- 多 SQLite session 管理、control/runtime/graph/event 模型边界和迁移测试通过。
