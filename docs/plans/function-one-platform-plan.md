...
> 执行模型：前后端并行，后端契约、runtime boundary 与 ToolProtocol 先行，Claude Code 风格 Workspace tools 基座先于正式 runtime 消费，deterministic runtime 先打通稳定链路，LangGraph runtime 作为正式编排路径
...
- 后端具备 FastAPI 服务、REST API、SSE、OpenAPI、领域模型、查询投影、多 SQLite 职责拆分、平台日志审计、运行状态机、人工介入、工作区工具、交付适配与 LangGraph 执行内核接入。
...
| --- | --- | --- | --- | --- |
| Week 1 | 工程与骨架基线 | 工具链、FastAPI app、错误模型、运行数据目录预检 | Vite React 骨架、路由、测试工具 | 后端健康检查、日志目录预检与前端控制台入口可运行 |
| Week 2 | 契约与持久化 | 枚举、Schema、多 SQLite、Alembic、log.db、TraceContext | API client、mock fixtures | Pydantic Schema、投影契约与日志审计契约可作为前后端输入 |
| Week 3 | 控制面可用 | Project、Session、Template、Provider、DeliveryChannel API、request context、RedactionPolicy、JSONL/log index、AuditService | Shell、设置弹窗、模板空态 | 控制面 OpenAPI、字段、日志与审计记录对齐 |
| Week 4 | Run 主链骨架 | Run 状态机、模板快照、GraphDefinition、StageArtifact、run trace | Workspace 页面、Run 分段基础展示 | draft、running 与基础阶段回放可用，run 级 trace 可追踪 |
...
| Week 7 | deterministic 与工具基座 | RuntimeEngine、deterministic runtime、ToolProtocol、Workspace tools 基座、工具审计、demo_delivery | 前端完整流程联调 | 不依赖真实模型跑通 demo_delivery，终态回放可用，工具协议、Claude Code 风格工具口径和审计不临时分叉 |
| Week 8 | LangGraph 正式路径 | Graph compiler、interrupt、checkpoint、Provider adapter、自动回归、模型/runtime 日志 | Runtime 错误态与状态呈现 | 内部测试/联调路径可在 deterministic 与 LangGraph runtime 间切换，正式用户运行只走 LangGraph；LangGraph 消费抽象 ToolProtocol 与已注册 `bash/read_file/edit_file/write_file/glob/grep` 或 fake 工具 |
                          ...
                          -> 交付快照门禁
                            -> 人工介入命令
                              -> ToolProtocol 与 ToolRegistry
                                -> 工作区工具基座
                                  -> deterministic runtime 与 demo_delivery 交付路径
...
| [01 工程基线与契约层](function-one-platform/01-foundation-and-contracts.md) | 工程基线、Schema 契约、数据库职责拆分、运行数据目录、log.db、TraceContext | B0.1, B0.2, F0.1, C1.1-C1.9, L0.1, L1.1-L1.2 |
| [02 控制面与工作台外壳](function-one-platform/02-control-plane-and-workspace-shell.md) | Project、Session、Template、Provider、DeliveryChannel、控制面日志审计、Shell、设置、模板空态 | C2.1-C2.7, L2.1-L2.4, F2.1-F2.6 |
| [03 Run 主链、投影与叙事流](function-one-platform/03-run-projection-and-feed.md) | PipelineRun、GraphDefinition、StageArtifact、Workspace Projection、SSE、Run/Stage 日志轻查询、Feed、Inspector | R3.1-R3.7, Q3.1-Q3.4, E3.1-E3.2, L3.1, F3.1-F3.7 |
| [04 人工介入与执行内核](function-one-platform/04-human-loop-and-runtime.md) | Runtime orchestration boundary、澄清、审批、暂停恢复终止、交付快照、命令审计失败语义、审计查询、deterministic runtime、demo_delivery、LangGraph、Provider、自动回归 | A4.0-A4.11, H4.1-H4.7, L4.1-L4.2, F4.1-F4.4, D4.0-D4.3 |
| [05 工作区、真实 Git 交付与硬化](function-one-platform/05-workspace-delivery-and-hardening.md) | ToolProtocol、Workspace tools、ChangeSet、git_auto_delivery、E2E、OpenAPI、日志轮转保留清理、日志审计回归包、回归硬化 | W5.0-W5.6, D5.1-D5.4, F5.1-F5.2b, V6.1-V6.7, L6.1-L6.2 |
...
| W5.2 | 文件工具 read_file/write_file/edit_file/glob | Week 7 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w52) |
...
- 核心枚举、Schema、数据库边界定稿。
- Pydantic Schema、投影契约与事件载荷可作为前端 mock 输入。
...
- ToolProtocol、ToolRegistry 与 Claude Code 风格 Workspace tools 基座完成，deterministic runtime 不使用临时工具接口。
...
- deterministic runtime 阶段推进、失败、终止和 `demo_delivery` 具备运行日志摘要与 trace 关联。
...
- 内部测试、前端联调与端到端验证可在 deterministic 与 LangGraph runtime 间切换；该切换不进入前端设置、项目设置、模板配置或平台环境变量，正式用户运行只走 LangGraph runtime，且 raw graph state 不进入产品 API。
...
| ToolProtocol 漂移导致 runtime、Provider adapter 与 delivery tool 绑定口径不一 | W5.0 前置固定 `ToolProtocol` 与 `ToolRegistry`，W5.2-W5.4 和 D5.1-D5.4 只实现具体工具，LangGraph 和 Provider adapter 只能消费抽象协议；Workspace tools 的正式契约名固定为 `bash`、`read_file`、`edit_file`、`write_file`、`glob`、`grep` |
