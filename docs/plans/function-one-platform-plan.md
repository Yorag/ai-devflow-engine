...
- 后端具备 FastAPI 服务、REST API、SSE、OpenAPI、领域模型、查询投影、多 SQLite 职责拆分、环境变量与可热重载配置设置分层、系统内置提示词资产管理、平台日志审计、运行状态机、人工介入、工作区工具、交付适配与 LangGraph 执行内核接入。
...
- `PromptAsset` Schema、系统内置提示词资产目录与 PromptRenderer 消费边界定稿。
...
| Week 2 | 契约与持久化 | 枚举、Schema、Project/Session 历史可见性、PlatformRuntimeSettings、运行快照 Schema、PromptAsset Schema、多 SQLite、Alembic、log.db、TraceContext | API client、mock fixtures | Pydantic Schema、投影契约、配置契约、提示词资产契约与日志审计契约可作为前后端输入 |
...
| Week 8 | LangGraph 正式路径 | Graph compiler、interrupt、checkpoint、PromptValidation、PromptRegistry、PromptRenderer、ContextEnvelope / ContextManifest、Provider adapter、AgentDecision、Stage Agent Runtime、自动回归、模型/runtime 日志 | Runtime 错误态与状态呈现 | 内部测试/联调路径可在 `deterministic test runtime` 与 LangGraph runtime 间切换，正式用户运行只走 LangGraph；LangGraph 消费 Context Management、PromptRegistry、PromptRenderer、AgentDecision、抽象 ToolProtocol 与已注册 `bash/read_file/edit_file/write_file/glob/grep` 或 fake 工具 |
                                    ...
                                    -> 工作区工具基座
                                      -> `deterministic test runtime` 与 demo_delivery 交付路径
                                        -> LangGraph 运行时与 Provider 适配器
                                          -> PromptValidation、PromptRegistry、PromptRenderer、ContextEnvelope / ContextManifest 与 AgentDecision
...
| [01 工程基线与契约层](function-one-platform/01-foundation-and-contracts.md) | 工程基线、Schema 契约、数据库职责拆分、Project/Session 历史可见性、EnvironmentSettings、PlatformRuntimeSettings、PromptAsset Schema、运行数据目录、log.db、TraceContext | B0.1-B0.3, F0.1, C1.1-C1.10, C1.10a, L0.1, L1.1-L1.2 |
...
| [04 人工介入与执行内核](function-one-platform/04-human-loop-and-runtime.md) | Runtime orchestration boundary、澄清、审批、暂停恢复终止、交付快照、命令审计失败语义、审计查询、`deterministic test runtime`、demo_delivery、LangGraph、Provider、PromptValidation、PromptRegistry、PromptRenderer、ContextEnvelope / ContextManifest、AgentDecision、Stage Agent Runtime、自动回归 | A4.0-A4.11, A4.8a-A4.8d, A4.9a-A4.9d, H4.1-H4.7, L4.1-L4.2, F4.1-F4.4, D4.0-D4.3 |
| [05 工作区、真实 Git 交付与硬化](function-one-platform/05-workspace-delivery-and-hardening.md) | ToolProtocol、ToolRegistry execution gate、统一错误码、Workspace tools、ChangeSet、后端测试 fixture、git_auto_delivery、E2E、OpenAPI、配置快照回归、提示词资产边界回归、日志轮转保留清理、日志审计回归包、回归硬化 | W5.0-W5.6, W5.0a-W5.0c, D5.1-D5.4, F5.1-F5.2b, V6.1-V6.8, L6.1-L6.2 |
...
| A4.8c | PromptRegistry 与系统提示词资产加载 | Week 8 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#a48c) |
| A4.8d | PromptRenderer 与消息序列渲染 | Week 8 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#a48d) |
