...
适用范围：
- 必须纳入实施计划的前端展示切片：`F2.3-F2.6`、`F3.3-F3.7`、`F4.1-F4.4`、`F4.3a`、`F5.1-F5.2b`。
...
| [04 人工介入、工具确认与执行内核](function-one-platform/04-human-loop-and-runtime.md) | Runtime orchestration boundary、澄清、审批、工具确认、暂停恢复终止、交付快照、命令审计失败语义、审计查询、`deterministic test runtime`、demo_delivery、LangGraph、Provider、Provider retry/circuit breaker、PromptValidation、PromptRegistry、PromptRenderer、ContextEnvelope / ContextManifest、AgentDecision、Stage Agent Runtime、自动回归 | A4.0-A4.11, A4.8a-A4.8d, A4.9a-A4.9e, H4.1-H4.7, H4.4a, L4.1-L4.2, F4.1-F4.4, F4.3a, D4.0-D4.3 |
| [05 工作区、真实 Git 交付与硬化](function-one-platform/05-workspace-delivery-and-hardening.md) | ToolProtocol、ToolRegistry execution gate、工具风险确认门禁、统一错误码、Workspace tools、ChangeSet、后端测试 fixture、git_auto_delivery、E2E、OpenAPI、配置快照回归、提示词资产边界回归、日志轮转保留清理、日志审计回归包、回归硬化 | W5.0-W5.6, W5.0a-W5.0d, D5.1-D5.4, F5.1-F5.2b, V6.1-V6.8, L6.1-L6.2 |
...
| W5.0d | Tool risk classifier 与 confirmation gate | Week 7-8 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w50d) |
...
| A4.9e | Provider retry、backoff 与 circuit breaker | Week 8 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#a49e) |
