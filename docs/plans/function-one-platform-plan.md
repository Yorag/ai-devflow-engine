...
- 后端具备 FastAPI 服务、REST API、SSE、OpenAPI、领域模型、查询投影、多 SQLite 职责拆分、环境变量与可热重载配置设置分层、平台日志审计、运行状态机、人工介入、工作区工具、交付适配与 LangGraph 执行内核接入。
...
- Walking skeleton 先行：先打通默认 Project、draft Session、首条需求创建正式 `PipelineRun`、模板快照、`GraphDefinition`、首条消息事件、初始 `StageRun`、workspace projection、前端读取与 SSE 增量，再逐步接入完整 runtime。
- Runtime boundary 先行：人工介入命令必须先依赖统一 runtime orchestration boundary，不允许先用 run 状态字段临时模拟中断、恢复或终止。
- 配置边界先行：`EnvironmentSettings`、`PlatformRuntimeSettings`、业务配置对象与运行快照必须在控制面、Run 启动和 runtime 消费前先固定；环境变量不承载 Provider、DeliveryChannel、模板运行配置、Agent 运行上限或日志策略。
- ToolProtocol 先行：正式 runtime、Provider adapter、Workspace tools 与 Delivery tools 只能消费抽象 `ToolProtocol` 和 `ToolRegistry`，不允许先接入临时工具函数或尚未实现的具体 delivery tool 实例；Workspace tools 参考 Claude Code 的工具工作方式，正式契约名固定为 `bash`、`read_file`、`edit_file`、`write_file`、`glob`、`grep`。
...
- `PlatformRuntimeSettings` 与运行快照语义定稿。
- LangGraph runtime 与 `deterministic test runtime` 的接口边界定稿。
...
适用范围：
- 必须纳入实施计划的前端展示切片：`F2.3-F2.6`、`F3.3-F3.7`、`F4.1-F4.4`、`F5.1-F5.2b`。
- 作为人工介入前端输入切片纳入实施计划：`H4.2`。
- 建立前端基线时纳入检查口径：`F0.1`。
- 系统回归与发布候选阶段纳入验收口径：`V6.2`、`V6.3`、`V6.6`、`V6.7`、`V6.8`。
...
| Week 2 | 契约与持久化 | 枚举、Schema、Project/Session 历史可见性、PlatformRuntimeSettings、运行快照 Schema、多 SQLite、Alembic、log.db、TraceContext | API client、mock fixtures | Pydantic Schema、投影契约、配置契约与日志审计契约可作为前后端输入 |
| Week 3 | 控制面可用 | Project、Session、Template、Provider、DeliveryChannel API、PlatformRuntimeSettings 管理服务、request context、RedactionPolicy、JSONL/log index、AuditService | Shell、项目/会话左栏、设置弹窗、模板空态 | 控制面 OpenAPI、字段、配置边界、历史可见性、日志与审计记录对齐 |
...
| Week 5 | 投影与实时更新 | Workspace Projection、Timeline、Inspector、SSE、run/stage 日志查询 | Narrative Feed、Inspector、SSE merge | 快照 + 增量一致，诊断日志查询不进入前端主路径 |
| Week 6 | 人工介入与 runtime 边界 | Runtime orchestration boundary、命令审计失败语义、审计查询、澄清、审批、暂停、恢复、终止、重新尝试、交付快照 gate | Approval Block、Composer lifecycle、重新尝试 UI | 人工介入命令只通过统一 runtime 边界推进，命令审计可追踪 |
| Week 7 | deterministic test 与工具基座 | RuntimeEngine、`deterministic test runtime`、ToolProtocol、统一错误码、ToolRegistry execution gate、Workspace tools 基座、工具审计、demo_delivery | 前端完整流程联调 | 不依赖真实模型跑通 demo_delivery，终态回放可用，工具协议、配置快照、错误码、Claude Code 风格工具口径和审计不临时分叉 |
| Week 8 | LangGraph 正式路径 | Graph compiler、interrupt、checkpoint、PromptValidation、ContextEnvelope / ContextManifest、Provider adapter、AgentDecision、Stage Agent Runtime、自动回归、模型/runtime 日志 | Runtime 错误态与状态呈现 | 内部测试/联调路径可在 `deterministic test runtime` 与 LangGraph runtime 间切换，正式用户运行只走 LangGraph；LangGraph 消费 Context Management、AgentDecision、抽象 ToolProtocol 与已注册 `bash/read_file/edit_file/write_file/glob/grep` 或 fake 工具 |
  ...
  -> 工程基线
    ...
    -> 枚举与 Schema 契约
      ...
      -> 多 SQLite 默认路径派生与持久化边界（control、runtime、graph、event、log）
        -> PlatformRuntimeSettings 与运行快照 Schema 契约
          ...
          -> 前端 API 客户端与模拟契约数据
            -> 前端工作台外壳、设置与模板交互
              -> Run 状态机、EventStore、模板快照、Provider/模型绑定快照、运行上限快照与 GraphDefinition
                ...
                -> 首个 Run 启动事务
                  -> StageArtifact
                    -> 查询投影（Workspace、Timeline、Inspector）
                    -> SSE、Run/Stage 日志轻查询与前端状态合并
                      -> Narrative Feed 与 Inspector 展示
                        -> Runtime 编排边界
                          -> 命令审计失败语义
                          -> 审计日志查询
                          -> 交付快照门禁
                                    ...
                                    -> 工作区工具基座
                                      -> `deterministic test runtime` 与 demo_delivery 交付路径
                                        -> LangGraph 运行时与 Provider 适配器
                                          -> PromptValidation、ContextEnvelope / ContextManifest 与 AgentDecision
...
| [01 工程基线与契约层](function-one-platform/01-foundation-and-contracts.md) | 工程基线、Schema 契约、数据库职责拆分、Project/Session 历史可见性、EnvironmentSettings、PlatformRuntimeSettings、运行数据目录、log.db、TraceContext | B0.1-B0.3, F0.1, C1.1-C1.10, L0.1, L1.1-L1.2 |
...
| [03 Run 主链、投影与叙事流](function-one-platform/03-run-projection-and-feed.md) | PipelineRun、模板快照、Provider/模型绑定快照、运行上限快照、GraphDefinition、StageArtifact、Workspace Projection、SSE、Run/Stage 日志轻查询、Feed、Inspector | R3.1-R3.4, R3.4a-R3.4b, R3.5-R3.7, Q3.1-Q3.4, E3.1-E3.2, L3.1, F3.1-F3.7 |
| [04 人工介入与执行内核](function-one-platform/04-human-loop-and-runtime.md) | Runtime orchestration boundary、澄清、审批、暂停恢复终止、交付快照、命令审计失败语义、审计查询、`deterministic test runtime`、demo_delivery、LangGraph、Provider、PromptValidation、ContextEnvelope / ContextManifest、AgentDecision、Stage Agent Runtime、自动回归 | A4.0-A4.11, A4.8a-A4.8b, A4.9a-A4.9d, H4.1-H4.7, L4.1-L4.2, F4.1-F4.4, D4.0-D4.3 |
| [05 工作区、真实 Git 交付与硬化](function-one-platform/05-workspace-delivery-and-hardening.md) | ToolProtocol、ToolRegistry execution gate、统一错误码、Workspace tools、ChangeSet、后端测试 fixture、git_auto_delivery、E2E、OpenAPI、配置快照回归、日志轮转保留清理、日志审计回归包、回归硬化 | W5.0-W5.6, W5.0a-W5.0c, D5.1-D5.4, F5.1-F5.2b, V6.1-V6.8, L6.1-L6.2 |
...
| C1.1 | 全局枚举与状态契约 | Week 2 | [ ] | 后端契约 | [01](function-one-platform/01-foundation-and-contracts.md#c11) |
...
| R3.4b | RuntimeLimitSnapshot 固化 | Week 4 | [ ] | 后端 | [03](function-one-platform/03-run-projection-and-feed.md#r34b) |
...
| W5.0 | ToolProtocol 与工具注册表 | Week 7 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w50) |
| W5.0a | 统一错误码字典与错误响应契约 | Week 7 | [ ] | 后端契约 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w50a) |
| W5.1 | WorkspaceManager 隔离工作区 | Week 7 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w51) |
| W5.0b | ToolRegistry execution gate | Week 7-8 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w50b) |
| W5.0c | 后端测试 fixture、fake provider 与 fake tool 契约 | Week 7-8 | [ ] | 后端测试 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w50c) |
| W5.2 | 文件工具 read_file/write_file/edit_file/glob | Week 7-8 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w52) |
| W5.3 | grep 工具 | Week 7-8 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w53) |
| W5.4 | bash 工具与白名单审计 | Week 7-8 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#w54) |
| A4.2 | deterministic test runtime 六阶段推进 | Week 7 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#a42) |
| A4.3 | deterministic test runtime 澄清与审批中断 | Week 7 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#a43) |
| A4.4 | deterministic test runtime 终态控制 | Week 7 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#a44) |
| D4.1 | Delivery base 与 DeliveryRecord | Week 7 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#d41) |
| D4.2 | demo_delivery adapter 与 delivery_result | Week 7 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#d42) |
| D4.3 | DeliveryResultDetailProjection 正式实现 | Week 7 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#d43) |
...
| A4.8a | PromptValidation 边界校验 | Week 8 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#a48a) |
| A4.8b | ContextEnvelope 与 ContextManifest Schema | Week 8 | [ ] | 后端契约 | [04](function-one-platform/04-human-loop-and-runtime.md#a48b) |
| A4.9 | LangChain Provider Adapter | Week 8 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#a49) |
| A4.9a | ContextEnvelope Builder 与 ContextManifest 记录 | Week 8 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#a49a) |
| A4.9b | Context Size Guard 与压缩过程记录 | Week 8 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#a49b) |
| A4.9c | AgentDecision Schema 与解析器 | Week 8 | [ ] | 后端 | [04](function-one-platform/04-human-loop-and-runtime.md#a49c) |
| A4.9d | Stage Agent Runtime 执行循环 | Week 8 | [ ] | 后端核心 | [04](function-one-platform/04-human-loop-and-runtime.md#a49d) |
...
| D5.1 | read_delivery_snapshot 与交付快照读取 | Week 10 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#d51) |
...
| D5.4 | git_auto_delivery 编排与 snapshot readiness 测试 | Week 10 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#d54) |
| F5.1 | 工具调用、Diff 与测试结果展示 | Week 9-10 | [ ] | 前端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#f51) |
| F5.2a | demo_delivery 结果展示 | Week 9 | [ ] | 前端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#f52a) |
| F5.2b | git_auto_delivery 结果展示 | Week 10 | [ ] | 前端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#f52b) |
...
| V6.8 | 配置边界与运行快照回归 | Week 12 | [ ] | 跨端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#v68) |
| L6.1 | 日志轮转与保留清理 | Week 12 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#l61) |
| L6.2 | 日志审计回归包 | Week 12 | [ ] | 后端 | [05](function-one-platform/05-workspace-delivery-and-hardening.md#l62) |
...
- `PlatformRuntimeSettings`、`RuntimeLimitSnapshot`、Provider/模型绑定快照 Schema 定稿，且明确 `compression_prompt` 是系统定义提示词，不是配置项。
...
- 设置弹窗只展示 DeliveryChannel 与 Provider 业务配置；不展示环境变量、SQLite 路径、平台运行上限、日志策略或 `deterministic test runtime`。
...
- Provider 与模型绑定快照、运行上限快照在 run 启动时固化；已启动 run 不读取最新 Provider 或平台运行设置改变语义。
- Session 删除和 Project 移除命令可用，且存在活动 run 时拒绝；Project 移除不删除本地项目文件夹、目标仓库、远端对象、提交或代码评审请求。
- 首条需求启动首个 run 时，PipelineRun、模板快照、GraphDefinition、首条消息事件和初始 StageRun 在同一服务事务中创建。
...
- Runtime orchestration boundary 定稿，澄清、审批、暂停、恢复和终止不得绕过该边界直接推进 run 状态；重新尝试只验证旧 GraphThread 终态并创建新 PipelineRun。
...
- Composer 输入语义、Approval Block、重新尝试 UI 完成设计硬化检查，禁用态、历史态、拒绝输入、危险操作和窄屏布局通过验收。

### Week 7

- ToolProtocol、ToolRegistry、统一错误码字典与 Claude Code 风格 Workspace tools 基座完成，`deterministic test runtime` 不使用临时工具接口。
...
- `deterministic test runtime` 可触发澄清、审批、失败和终止。
...
- ContextEnvelope、ContextManifest、Context Size Guard、压缩过程记录、AgentDecision Schema 与 Stage Agent Runtime 执行循环可用；模型调用不得绕过 Context Management，工具调用不得绕过 ToolRegistry，阶段结果不得绕过 StageArtifact。
...
- 配置边界、运行快照、热重载不回写已启动 run、前端设置边界进入系统回归。
...
- 配置边界与运行快照回归覆盖 EnvironmentSettings、PlatformRuntimeSettings、RuntimeLimitSnapshot、Provider/模型绑定快照和前端设置边界。
...
| 环境变量膨胀为业务配置中心 | B0.3 固定 `EnvironmentSettings` 只服务启动、路径、前后端连接和凭据引用解析；C1.10/C2.8 把运行设置放入 `PlatformRuntimeSettings`，Provider、DeliveryChannel 和模板运行配置仍走业务 API 或模板编辑 |
| 热重载设置改变已启动 run 语义 | R3.4a/R3.4b 在 run 启动时固化 Provider/模型绑定快照与 `RuntimeLimitSnapshot`，A4.1-A4.11 只消费本次 run 快照，V6.8 做回归 |
| `compression_prompt` 被实现为用户配置或热重载配置 | C1.10 固定其为系统定义提示词，A4.9 只记录系统定义版本引用，V6.8 检查其不进入环境变量、设置弹窗或配置 API |
| 前端设置弹窗暴露平台内部配置 | F2.4 只展示 DeliveryChannel 与 Provider，F2.6 只展示模板允许字段，V6.8 回归环境变量、SQLite 路径、运行上限、日志策略和 `deterministic test runtime` 不进入普通设置 |
...
| 错误码散落导致前后端和工具错误不可回归 | B0.2 建立基础错误契约，W5.0a 扩展统一错误码字典，C2.8、R3.4a/R3.4b、A4.8a-A4.9d、W5.0b-W5.4、D5.1-D5.4 和 L6.2/V6.6 只能引用该字典，不得在路由、工具或 runtime 中散落自由文本错误码 |
| 用户 prompt 覆盖平台边界 | A4.8a 在模板保存和 run 启动前执行 PromptValidation；A4.9a 只把通过校验且已固化的 `system_prompt` 作为低权威 `agent_role_prompt` 放入 ContextEnvelope |
| 上下文组装绕过可信边界或丢失可追溯性 | A4.8b 固定 ContextEnvelope / ContextManifest Schema，A4.9a/A4.9b 负责构建、尺寸守卫、压缩和 `StageArtifact.process` 记录，Inspector 只能读取 StageArtifact 或稳定引用 |
