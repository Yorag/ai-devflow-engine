...
11. `DeliveryChannel`
代码托管与交付通道定义对象，用于表达目标托管平台、仓库标识、默认分支、代码评审请求类型以及对应的交付策略。
...
14. `AcceptanceCriteria`
结构化验收标准对象，用于贯穿需求、方案、代码、测试与交付。

15. `ClarificationItem`
...
3. `工具调用模型`
Agent 不直接绑定零散函数签名，而是通过统一的 Tool 泛型接口调用能力。V1 的工具能力分为两类：
...
- 交付工具负责封装本地 Git 与远端托管平台差异，规格层不直接暴露 `git`、`gh`、`glab` 等实现命令名
...
5. `模型供应商可切换`
- V1 模型供应商集合固定为两个：
  - `火山引擎`：只要求提供 `api_key` 与 `model_id`
  - `OpenAI Completions` 兼容接口：要求提供 `base_url`、`api_key` 与 `model_id`
...
8. `多 Agent 使用原则`
多 Agent 协作只在已定义的方案阶段结构中启用：
- `Solution Design`：`Solution Design Agent -> Solution Validation Agent` 串行结构
- `Code Generation` 与 `Test Generation & Execution`：保持阶段串行执行，顺序固定为先 `Code Generation`，后 `Test Generation & Execution`
...
- `Delivery Integration Agent`
  ...
  - 输入：已通过审查的 `code_changeset`、`review_report`、`test_execution_result`、`delivery_channel`
...
系统能力必须遵循统一工具模型，而不是把“读文件、写文件、搜索文件、执行命令、Git 操作、代码评审请求创建”直接暴露为零散函数。V1 的工具能力拆分如下：
...
其中：
- `Workspace Tools` 负责文件系统、上下文读取、补丁应用、命令执行与 diff 生成
- `SCM / Delivery Tools` 负责分支准备、提交意图记录、远端推送、PR/MR 创建以及托管平台信息读取
- `SCM / Delivery Tools` 的实现可以基于 `git`、`gh`、`glab` 或托管平台 API，但这些实现细节不进入规格层工具命名
...
该设计参考 Claude Code 的工作区工具思路，但刻意收缩到最核心的一组工作区与交付工具，以保证实现可控、行为稳定、便于运行说明。
...
2. `Agent 编排与执行`
由 `Agent Role Registry`、`Agent Runtime`、`LLM Provider Adapter`、`Workspace & Tool Service`、`SCM & Delivery Adapter` 覆盖。
...
1. `多 Agent 协作`
- 用于在可拆解阶段降低遗漏和补足质量风险
- V1 仅在 `Solution Design` 阶段采用 `Solution Design Agent -> Solution Validation Agent` 串行协作
- `Code Generation` 与 `Test Generation & Execution` 在 V1 中必须保持串行执行，先产出代码变更，再生成并执行测试
...
3. `SCM / Delivery Tools` 仅实现五个核心工具：
- `prepare_branch`
- `record_commit_intent`
- `push_branch`
- `create_code_review_request`
...
- `Code Generation` 与 `Test Generation & Execution` 必须串行执行，先代码生成，后测试生成与执行
- 其他阶段使用单 Agent
...
4. Reject 后能带理由回退并重跑
5. Agent 可读取代码库上下文
6. 支持在两个模型供应商之间切换：`火山引擎` 与 `OpenAI Completions` 兼容接口
...
15. 交付阶段具备分支准备、提交说明生成、推送分支和 MR/PR 创建或描述生成能力
