...
- OpenAPI 文档必须覆盖功能一全部核心 REST 接口，包括 `Project`、`Session`、`PipelineTemplate`、`Provider`、项目级 `DeliveryChannel`、`PipelineRun` 生命周期、审批、工具确认、控制条目、Inspector、交付结果、预览目标查询、运行日志轻查询与审计日志查询
...
39. 能把 `SolutionDesignArtifact.implementation_plan` 作为 `Solution Design` 阶段产物输出，并供代码生成、测试生成执行和代码评审阶段按稳定任务标识引用。
40. 能在工具执行前统一执行风险分级，对高风险工具动作创建 `ToolConfirmationRequest`，对被阻断动作返回结构化拒绝错误，并保证工具确认不创建 `ApprovalRequest` 或 `ApprovalDecision`。
41. 能在 `Test Generation & Execution` 阶段读取项目说明、依赖声明和脚本配置来识别测试环境与依赖缺失，并在执行高风险测试、构建、格式化、依赖安装或环境修改命令前进入工具确认。
42. 能对 Provider 超时、网络错误和限流执行本次 run 固化的指数退避重试策略，对连续失败执行熔断，并把重试轨迹、熔断状态和失败影响投影到 Narrative Feed 或 Inspector。
43. 能保证新建 `Session` 不自动读取其他会话的历史 run、历史产物、历史审批、历史工具确认或历史工具过程作为 Agent 长期记忆；历史会话只作为回看、追溯、诊断和审计对象。
