...
- 模板快照、Provider 与模型绑定快照、执行图定义与交付通道快照一旦绑定到某次运行，不得再被运行外部修改
...
25. 能在 run 启动时固化 Provider 与模型绑定快照，并在 `ContextManifest` 与 `model_call_trace` 中引用本次调用使用的快照。
26. 能在暂停、终止和运维恢复时按安全点、`GraphCheckpoint`、阶段恢复游标和工作区快照引用恢复或失败，不重复执行已确认成功的副作用工具调用。
27. 能记录副作用工具调用的调用意图、协调键、实际副作用、协调状态和审计引用；`bash` 间接造成的工作区变更必须进入 `file_edit_trace`、`command_trace` 与 `ChangeSet` 构建输入。
28. 能在代码评审自动回归或 `code_review_approval` 拒绝后沿用当前 run 的现有工作区改动继续修补，并在 Inspector 中区分原有改动与本轮修补。
29. 能把用户消息、附件、仓库文件、工具观察、测试输出和远端交付返回作为不可信上下文处理，并在 `ContextManifest` 中记录来源、可信级别和边界处理结果。
30. 能对用户可编辑 `system_prompt` 执行边界校验、长度校验和运行快照固化，并保证其不得覆盖阶段契约、工具边界、审批边界、交付边界或输出 Schema。
31. 能在 `ContextManifest` 中记录最终渲染提示词或消息序列引用、hash、模板版本、提示词片段来源以及模型绑定快照引用。
32. 能按 `AgentDecision` 或等价结构化决策协议解析 Stage Agent 输出，并拒绝自由文本推进状态、执行工具、创建审批或触发交付。
33. 能识别模型调用异常、非法 tool call、输出 Schema 不匹配和无进展 ReAct 循环，并按修复、受控重试或 run 失败规则写入阶段过程记录。
34. 能确保需求澄清只在阻塞性缺失信息时触发，并记录缺失事实、影响范围、关联引用和回答后需更新的结构化字段。
