...
功能一 V1 后端必须建立统一配置管理边界，并明确区分 `EnvironmentSettings`、`PlatformRuntimeSettings`、业务配置对象与运行快照。
...
- 不得包含 Provider 的 `base_url`、`model_id`、能力声明、模板角色绑定、`system_prompt`、交付仓库、目标分支、代码评审请求类型、交付模式、Agent 循环上限、日志保留策略或 `compression_prompt`
...
- 任何 Agent 编排切片必须同时经过 Context Management、PromptValidation、ToolRegistry 和 StageArtifact 四个边界；模型调用不得绕过 `ContextEnvelope`，提示词不得绕过 `PromptValidation`，工具调用不得绕过 `ToolRegistry`，阶段结果不得绕过 `StageArtifact`
...
33. 能对用户可编辑 `system_prompt` 执行边界校验、长度校验和运行快照固化，并保证其不得覆盖阶段契约、工具边界、审批边界、交付边界或输出 Schema。
34. 能在 `ContextManifest` 中记录最终渲染提示词或消息序列引用、hash、模板版本、提示词片段来源以及模型绑定快照引用。
35. 能按 `AgentDecision` 或等价结构化决策协议解析 Stage Agent 输出，并拒绝自由文本推进状态、执行工具、创建审批或触发交付。
36. 能识别模型调用异常、非法 tool call、输出 Schema 不匹配和无进展 ReAct 循环，并按修复、受控重试或 run 失败规则写入阶段过程记录。
37. 能确保需求澄清只在阻塞性缺失信息时触发，并记录缺失事实、影响范围、关联引用和回答后需更新的结构化字段。
