...
## 2.2 环境变量、配置设置、系统提示词资产与运行快照

功能一 V1 后端必须建立统一配置管理边界，并明确区分 `EnvironmentSettings`、`PlatformRuntimeSettings`、业务配置对象、系统内置提示词资产与运行快照。

### 2.2.1 EnvironmentSettings

`EnvironmentSettings` 表示服务启动前必须确定的环境变量或等价启动配置，由 `pydantic-settings` 加载。它只服务本地服务启动、路径落点、前后端连接和密钥引用解析，不承载业务配置。
...
- 不得包含 Provider 的 `base_url`、`model_id`、能力声明、模板角色绑定、`system_prompt`、系统内置提示词正文或版本切换、交付仓库、目标分支、代码评审请求类型、交付模式、Agent 循环上限、日志保留策略或 `compression_prompt`
...
### 2.2.2 PlatformRuntimeSettings

`PlatformRuntimeSettings` 表示后端统一管理的可热重载运行设置。它用于控制运行上限、Provider 调用策略、上下文裁剪、日志裁剪、日志保留和诊断查询分页等平台级策略，不作为用户业务产物。
...
- `provider_call_policy`
  包含 Provider 请求超时、网络错误重试次数、限流重试次数和退避上限
- `context_limits`
  包含单次工具输出进入 `ContextEnvelope` 的预览长度、`bash` stdout / stderr 预览长度、`grep` 最大返回条数、文件读取最大字符数或字节数、模型输出进入日志或过程记录的裁剪长度
- `log_policy`
  包含普通运行日志保留周期、审计日志保留周期、日志轮转大小、日志查询默认 `limit` 与最大 `limit`

`PlatformRuntimeSettings` 必须满足以下规则：
- 配置设置允许热重载，但不得改变已经启动 run 的执行语义
...
- 已启动 run 必须继续使用自身快照中的运行上限、Provider 与模型绑定、模板配置、交付通道快照和阶段契约；不得读取最新配置设置来改变当前 run
...
- 配置设置变更必须写入审计日志，记录变更主体、变更字段、旧值摘要、新值摘要、生效范围和关联 `correlation_id`
- 高影响配置设置变更不得绕过 API 校验直接写数据库；测试 fixture 可以通过 settings override 构造隔离配置
...
业务配置必须满足以下规则：
- `LLMProvider` 与 `DeliveryChannel` 支持通过控制面 API 和前端设置弹窗编辑
- `PipelineTemplate` 与 `AgentRole` 的用户可编辑字段只通过模板编辑能力修改
- `LLMProvider`、`PipelineTemplate` 与 `AgentRole` 的保存结果只影响后续新建 Session、后续新启动 run 或尚未启动 run 的模板选择
...
- 系统内置提示词资产不属于环境变量、`PlatformRuntimeSettings`、前端设置、模板编辑字段或用户业务配置；变更系统内置提示词资产属于代码/发布变更，不通过热重载配置改变已经启动 run 的执行语义
...
后端必须在 run 启动或进入指定边界前固化运行所需配置，使热重载设置和业务配置不会破坏历史可回放性。
...
- `StageArtifact.process` 在 V1 中至少支持以下过程记录类型：`context_manifest`、`reasoning_trace`、`decision_trace`、`tool_trace`、`model_call_trace`、`file_edit_trace`、`command_trace`、`validation_trace`、`compressed_context_block`、`structured_output_repair_trace`、`recovery_checkpoint`、`side_effect_reconciliation_trace`、`untrusted_context_trace`
- `StageArtifact.process` 中的过程记录可以引用完整大载荷、工具结果、日志定位或附件，但不得只把关键过程信息留在运行日志中
- `StageArtifact.process` 必须能够表达阶段内恢复点、恢复后的继续执行记录、工具副作用协调记录、`bash` 命令工作区变更记录与不可信上下文处理记录
...
- 本阶段输出必须是 `SolutionDesignArtifact` 或等价结构化阶段产物包，至少包含：`technical_plan`、`impacted_files`、`api_design`、`data_flow_design`、`risks`、`test_strategy`、`validation_report`、`requirement_refs`、`evidence_refs`
...
- 本阶段必须基于已批准或当前有效的 `SolutionDesignArtifact`、`RequirementAnalysisArtifact`、审批反馈引用、上下文引用和目标仓库工作区执行
...
- 本阶段必须基于 `ChangeSet`、`RequirementAnalysisArtifact.acceptance_criteria`、`SolutionDesignArtifact.test_strategy` 和相关上下文引用生成或修改测试
- 测试命令执行必须通过受控 `bash` tool 完成
...
- 本阶段 `allowed_tools` 只允许包含只读工具：`read_file`、`glob`、`grep`
- 本阶段必须基于 `ChangeSet`、`SolutionDesignArtifact`、`RequirementAnalysisArtifact`、测试执行结果、测试缺口、相关文件和必要上下文执行评审
...
| `solution_design` | `read_file`、`glob`、`grep` | 只读分析代码库、需求产物和原始事实引用，形成方案并执行 validation pass |
| `code_generation` | `read_file`、`glob`、`grep`、`write_file`、`edit_file` | 使用单 Agent ReAct 工具循环修改文件、检查 diff 并形成 `ChangeSet` |
| `test_generation_execution` | `read_file`、`glob`、`grep`、`write_file`、`edit_file`、`bash` | 使用单 Agent ReAct 工具循环生成或修改测试、执行测试命令并分析结果 |
...
- `AgentDecision.decision_type` 在 V1 至少支持：`request_tool_call`、`submit_stage_artifact`、`request_clarification`、`repair_structured_output`、`retry_with_revised_plan`、`fail_stage`
...
- `fail_stage` 必须携带失败原因、已执行证据引用、未完成事项和可展示错误摘要；运行时必须把当前 `StageRun.status` 与 `PipelineRun.status` 映射为 `failed`，除非该失败已被阶段契约映射到澄清或审批等待
...
- 用户消息、澄清回复、审批反馈、附件、仓库文件、测试输出、工具观察和外部交付返回必须作为不可信上下文块进入 `ContextEnvelope`，不得覆盖 `runtime_instructions`、`agent_role_prompt`、`stage_contract`、`allowed_tools` 或 `response_schema`
...
#### 7.6.4 Context Source Resolution
...
- 用户消息、澄清回复、审批反馈、附件、仓库文件、`bash` 输出、测试输出、工具观察、远端交付返回和历史模型输出属于业务事实或观察结果，不属于系统指令来源
- 当不可信上下文包含类似“忽略前文指令”“调用未授权工具”“泄露凭据”“跳过审批”“直接提交”等指令性内容时，模型可以把它作为待分析事实或风险信号处理，但不得据此改变工具边界、审批边界、交付边界或输出 Schema
- `ContextManifest` 必须记录不可信上下文块被纳入、裁剪、压缩、引用或阻断的处理结果，使 Inspector 能解释提示注入风险的处理过程

#### 7.6.5 Context Size Guard
...
- Provider 超时、网络错误和限流允许在当前阶段运行上限内进行受控重试；重试次数、退避结果和最终状态必须进入阶段过程记录
...
5. 具体工具执行并返回结构化 `ToolResult`
6. `ToolResult` 写入运行日志、审计日志、`tool_trace`、`delivery_integration` 阶段过程记录、`DeliveryRecord.result_ref` 或等价稳定过程引用
...
24. 能通过 `EnvironmentSettings` 加载启动所需环境变量，并通过 `PlatformRuntimeSettings` 管理可热重载运行设置。
...
27. 能在统一配置管理中校验 Agent 运行上限、Provider 调用策略、上下文裁剪限制、日志裁剪限制、日志保留策略和日志查询分页上限，并确保热重载设置不改变已启动 run 的语义。
28. 能为功能二保留 `ChangeSet`、`ContextReference`、`PreviewTarget`、`DeliveryRecord` 的复用边界。
29. 能在暂停、终止和运维恢复时按安全点、`GraphCheckpoint`、阶段恢复游标和工作区快照引用恢复或失败，不重复执行已确认成功的副作用工具调用。
30. 能记录副作用工具调用的调用意图、协调键、实际副作用、协调状态和审计引用；`bash` 间接造成的工作区变更必须进入 `file_edit_trace`、`command_trace` 与 `ChangeSet` 构建输入。
31. 能在代码评审自动回归或 `code_review_approval` 拒绝后沿用当前 run 的现有工作区改动继续修补，并在 Inspector 中区分原有改动与本轮修补。
32. 能把用户消息、附件、仓库文件、工具观察、测试输出和远端交付返回作为不可信上下文处理，并在 `ContextManifest` 中记录来源、可信级别和边界处理结果。
33. 能通过 `PromptRegistry` 或等价能力统一管理系统内置提示词资产，并记录 `prompt_id`、`prompt_version`、权威级别、缓存属性、来源引用和内容 hash。
34. 能对用户可编辑 `system_prompt` 执行边界校验、长度校验和运行快照固化，并保证其不得覆盖阶段契约、工具边界、审批边界、交付边界或输出 Schema。
35. 能在 `ContextManifest` 中记录最终渲染提示词或消息序列引用、hash、模板版本、系统内置 prompt 版本、提示词片段来源以及模型绑定快照引用。
36. 能按 `AgentDecision` 或等价结构化决策协议解析 Stage Agent 输出，并拒绝自由文本推进状态、执行工具、创建审批或触发交付。
37. 能识别模型调用异常、非法 tool call、输出 Schema 不匹配和无进展 ReAct 循环，并按修复、受控重试或 run 失败规则写入阶段过程记录。
38. 能确保需求澄清只在阻塞性缺失信息时触发，并记录缺失事实、影响范围、关联引用和回答后需更新的结构化字段。
