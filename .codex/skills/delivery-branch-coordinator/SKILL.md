...
description: "Use when the user asks from main to coordinate Delivery Branch Plan batches, identify claimable branches, prepare worktree launch instructions, or manage branch-status updates around PR/MR review."
---
...
在主会话协调 `Delivery Branch Plan`：选择可认领 batch、标记 `claimed`、生成 worktree 命令和 worker 启动提示词，并在 PR/MR 合入后更新 `merged`。本技能不执行 slice；分支内实施仍由 `slice-workflow` 控制。
...
- **Discovery mode**：只找出下一批可认领 batch，并输出原因、风险和命令草案。
- **Claim mode**：用户选定 batch 后，只把这些 batch 的 `Status` 从 `planned` 改为 `claimed`。
- **Worker launch mode**：为已认领 batch 输出 worktree 命令和 worker 启动提示词。
- **Merge coordination mode**：PR/MR 已通过后，在主分支 review、合入，并只在合入完成后把 batch 标记为 `merged`。

## Candidate Selection

解析 `docs/plans/function-one-delivery-branch-plan.md` 表格。一个 batch 只有同时满足以下条件才可认领：

- `Status` 是 `planned`。
- `前置门槛` 为 `无`，或列出的每个 batch id 都存在且 `Status` 是 `merged`。
- `交付分支` 没有被其它 worktree 使用，且本地同名 branch 不存在；如果存在，报告表格状态与 Git 状态不一致，不要创建重复分支。
...
如果没有可认领 batch，报告阻塞原因：前置未 `merged`、状态不是 `planned`、branch/worktree 已存在、冲突点重叠或表格结构异常。
...
- Active batches: <claimed / ready_for_review>
...
对每个已认领 batch，输出 PowerShell 命令。路径使用分支名的安全目录名：把 `/` 替换为 `-`。
...
如果 branch 已存在但没有 worktree，不要自动复用；报告 Git 状态不一致，并让用户决定是绑定已有 branch、删除旧 branch，还是修正表格状态。
...
每次只推进一个 slice。完成当前 slice 后，更新对应 platform plan、split plan、implementation plan 和当前 batch 状态，并给出验证证据。如果 batch 仍有未完成 slice 且没有阻塞项，保持 claimed，停止并报告下一个继续提示词。
...
如果当前 branch 对应 batch 状态仍是 planned，停止并报告该分支没有经过主会话认领。不要自行把 planned 改为 claimed。
```
...
- 本地已有同名 branch 或 worktree，且表格状态仍是 `planned`。
- 多个待认领 batch 共享明确冲突点。
...
- 要求 worker 把 `claimed` 改为 `in_progress`；`claimed` 已覆盖已认领、worktree 已创建和正在实施。
