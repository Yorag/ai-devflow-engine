...
- claim `Status` 是 `claimed`，不是 `queued`、`implemented`、`mock_ready`、`integrated`、`done` 或 `blocked`。
...
- acceleration execution plan、platform plan、split plan 和 implementation plan 的最终追踪更新。
- 最新验证结论、完成声明和用户汇报。
...
- Worker evidence report 可以声明 `implemented`、`mock_ready` 或 `blocked`。
- `implemented` 只表示 claim 范围代码和 focused tests 已完成，等待 integration ingest。
- `mock_ready` 只表示基于冻结契约、fixture 或 mock 完成可验证部分；不得视为最终完成。
...
- 代码评审结果和修复。
- 验证命令、退出码和关键输出。
- 已做 worker evidence 更新；若未更新，说明原因。
- 剩余风险或阻塞项。
- 是否建议 commit 批准请求。
