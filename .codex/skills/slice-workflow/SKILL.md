...
- 不要在 worker 分支更新中央 Claim Ledger、platform plan 或 split plan 的最终完成状态；这些只由主协调会话在 integration checkpoint 后统一更新。
...
切片选择阶段默认只读取 acceleration execution plan、platform plan、相关 split-plan 细则、当前分支名和当前 claim 行。三个当前 specs 在语义不清、发现冲突，或已选切片触及对应契约时读取相关章节。
...
3. 选择且只选择当前 acceleration claim 指定的切片：其 claim 已由主协调会话分配、当前分支匹配 lane、依赖或 start gate 满足，且自身状态尚未完成。
...
在切片选择前，通过 `git branch --show-current` 读取当前分支，并检查 `docs/plans/function-one-acceleration-execution-plan.md`。

当前请求必须提供 claim id。claim id 可以来自用户提示词或 worker prompt。没有 claim id 时停止；worker 不得自行从 queue 抢任务。

对 claim 执行以下 gate：

- Claim Ledger 中存在该 claim。
...
- claim 的 `Lane` status 是 `claimed` 或 `integrating`；`planned` 只能由主协调会话 claim 后再执行。
...
每次只执行 claim 指定的一个 task slice。不要把边界设定任务与无关实现合并。不要执行同一 lane queue 中的下一个 task，除非主协调会话分配了新的 claim。
...
不得在 worker 分支自动扫描全局任务表选择下一个切片。只有主协调会话通过 `acceleration-workflow` claim 的 task 才能执行。
...
- claim id 缺失、未知、状态不可执行，或与当前分支不匹配。
...
- Worker evidence report 可以声明本地结果 `reported` 或 `blocked`，并写明提交后预期进入 `implemented` 还是 `mock_ready`。
- `reported` 表示本地 worktree 已写入 evidence report、implementation plan、代码 / 测试 diff 和验证记录；它只支持主协调会话本地读取，不是可合入 checkpoint。
- `implemented` 由主协调会话在确认 worker branch checkpoint commit 后写入 Claim Ledger；该 commit 必须包含 claim 范围代码、测试、implementation plan 和 evidence report。
- `mock_ready` 由主协调会话在确认 worker branch checkpoint commit 后写入 Claim Ledger；该 commit 只表示基于冻结契约、fixture 或 mock 完成可验证部分，不得视为最终完成。
- `blocked` 必须包含阻塞原因、已验证事实和建议 owner。
- Worker 不在 evidence report 中声明权威 Worker HEAD；Worker HEAD 由主协调会话在 ingest 时读取分支提交并写入 Claim Ledger。
- Worker 不得把 platform plan 或 split plan 标记为 `[x]`。
- Worker 不得更新 `docs/plans/function-one-acceleration-execution-plan.md` 的 Claim Ledger。
- integration checkpoint 后由主协调会话统一更新 platform plan、split plan 和 Claim Ledger。
...
- claim id 缺失、未知、状态不可执行或与当前分支不匹配。
...
- 剩余风险或阻塞项。
- 是否建议 commit 批准请求；如果建议，说明提交后预期由主协调会话 ingest 为 `implemented` 或 `mock_ready`。
